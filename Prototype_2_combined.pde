...
String[] defaultSuggestions  = {"the", "and", "to", "a"};
...
void drawHomeKeyboard()
{
  ...
  for (int row = 0; row < 4; row++)
  {
    for (int col = 0; col < 3; col++)
    {
      ...
      else if (action == ACTION_SUGGEST_0 || action == ACTION_SUGGEST_1)
      {
...
void handleTapRelease()
{
  ...
  if (activeTapAction == ACTION_SUGGEST_0 || activeTapAction == ACTION_SUGGEST_1)
  {
    ...
    if (suggestionOptionCount(idx) > 0)
      activeSuggestionGroup = idx;
  }
}
  ...
  if (bigramFreq.containsKey(prevWord))
  {
    ...
    for (String w : nextWords.keySet())
    {
      if (w.startsWith(prefix))
        addCandidate(w, nextWords.get(w), best, freqs);
    }
  }

  // Backfill with simple word frequencies
  for (String w : wordFreq.keySet())
  {
    if (w.startsWith(prefix))
      addCandidate(w, wordFreq.get(w), best, freqs);
  }

  for (int i = 0; i < 4; i++) {
    result[i] = best[i].equals("") ? defaultSuggestions[i] : best[i];
  }
  return result;
}
...
void addCandidate(String w, int f, String[] best, int[] freqs) {
  for (int i = 0; i < 4; i++) {
    if (w.equals(best[i])) return; // Prevent duplicates
  }
  for (int i = 0; i < 4; i++) {
    if (f > freqs[i]) {
      for (int j = 3; j > i; j--) {
...
String actionLabel(int action)
{
  ...
  if (action == ACTION_SUGGEST_0) return suggestionGroupPreviewLabel(0);
  if (action == ACTION_SUGGEST_1) return suggestionGroupPreviewLabel(1);
...
{
  // Uses the NEW wider positions just for the drag screen
  float[] pos = selectionSlotPosition(groupLength, index);
  
  // Reduced margin to push them closer to the corners/edges
...
// Original tight slot positions — used for the mini preview on the home keys
...
// New wider slot positions — used when holding down and dragging to pick a letter
float[] selectionSlotPosition(int groupLength, int index)
{
  ...
  // For 3-letter groups, 0=top-left, 1=top-right, 2=bottom-center
