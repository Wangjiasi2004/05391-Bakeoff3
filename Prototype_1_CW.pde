...
void drawSelectionMode()
{
  ...
  float bandHeight = sizeOfInputArea / group.length();

  textAlign(CENTER, CENTER);

  for (int i = 0; i < group.length(); i++)
  {
    float y = inputTop() + i * bandHeight;
    boolean isHovered = i == hoveredIndex;

    fill(isHovered ? color(246, 206, 92) : color(95));
    rect(inputLeft() + 6, y + 4, sizeOfInputArea - 12, bandHeight - 8, 12);

    fill(isHovered ? color(20) : color(248));
    textSize(group.length() == 4 ? 30 : 34);
    text(group.charAt(i), inputCenterX(), y + bandHeight / 2);
  }

  fill(250);
  textSize(9);
  text("hold " + group + " and slide to a letter", inputCenterX(), inputTop() + sizeOfInputArea - 8);
}
...
String[] computeVisibleSuggestions()
{
  ...
  if (prefix.length() > 0)
  {
    SuggestionBucket bucket = prefixSuggestions.get(prefix);
    fillFromBucket(next, bucket, prefix);
    return next;
  }

  String previousWord = lastCommittedWord();
  if (previousWord.length() > 0)
  {
    SuggestionBucket bucket = nextWordSuggestions.get(previousWord);
    fillFromBucket(next, bucket, "");
  }
...
{
  String group = GROUPS[activeGroup];
  float relativeY = constrain(dragY - inputTop(), 0, sizeOfInputArea - 0.001);
  return (int)(relativeY / (sizeOfInputArea / group.length()));
}
...
void loadPrefixSuggestions()
{
  ...
  try
  {
    while ((line = reader.readLine()) != null && loadedWords < MAX_UNIGRAM_WORDS)
    {
      ...
      if (defaultIndex < defaultSuggestions.length)
        defaultSuggestions[defaultIndex++] = word;
...
{
  String[] words = new String[SUGGESTION_SLOTS];
  long[] counts = new long[SUGGESTION_SLOTS];
