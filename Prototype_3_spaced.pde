...
void drawHomeKeyboard()
{
  ...
  // Draw Top Row (Custom Column Widths: Del=25%, Word=35%, Letter=20%, Chunk=20%)
  ...
  // Draw Rows 1 to 3 (3 Columns: Letters, Space)
...
// Full-screen suggestion picker (Word only, stacked vertically)
void drawSuggestionDragMode()
{
  ...
  fill(250);
  textSize(9);
  text("slide to pick word", inputCenterX(), inputTop() + sizeOfInputArea - 8);
}
...
void applySuggestion(String word, int groupIndex)
{
  ...
  if (groupIndex > 0)
  {
    // For single letters and chunks, ALWAYS append without adding a space
    currentTyped += word; 
  }
  else
  {
    // For full words (group 0), replace the current prefix and add a space
...
// ─────────────────────────────────────────────────────────────────────────────
// Prefix-Aware Suggestions (Dictionary Derived)
// ─────────────────────────────────────────────────────────────────────────────
...
String[] computeVisibleSuggestions()
{
  ...
  if (prefix.length() == 0) 
  {
    // State A: Starting a word (Immediately after space)
    ...
    // Dynamically find best starting letters from context, or default
  ...
  {
    // State B: Mid-word. Aggregating from valid words sharing the prefix
    ...
    if (prevWord.length() > 0 && bigramFreq.containsKey(prevWord))
    {
      ...
      for (String w : nextWords.keySet()) {
         if (w.startsWith(prefix) && w.length() > prefix.length()) {
             possibleWords.put(w, nextWords.get(w) * 1000); // Prioritize context
         }
      }
    }
    ...
    // prevent exact identical options from confusing user
...
// Top Row custom width helpers (0=Del, 1=Word, 2=Letter, 3=Chunk)
...
void loadLanguageModel() { 
  loadFrequencies(); 
  loadBigrams(); 
  // No longer loading the external letter N-Grams since we compute them from real words!
}
